...
class Tetris:
    ...
    def next_block(self):
        self.nextBlock = Block(3, 0, random.randint(0, 6))
        ...
        for i in range(4):
            for j in range(4):
                if i * 4 + j in self.block.image():
                    if (
                        i + self.block.y > self.height - 1
                        or j + self.block.x > self.width - 1
                        or j + self.block.x < 0
                        or self.field[i + self.block.y][j + self.block.x] > 0
                    ):
                        intersection = True
        return intersection
...
def play_background_music():
    pygame.mixer.music.load("noises/tetris_bg_music.mp3")
...
def startGame():
    ...
    while not done:
        ...
        if game.state == "gameover":
            ...
            screen.blit(text_game_over1, [250, 400])
...
while run:
    ...
    label = font.render("Press any key to begin!", True, "white")
